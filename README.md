...
- `/garde` : `follow-radius` (distance maximale avant rappel), `comfort-distance` (distance de suivi), `protection-radius` (rayon de poursuite), `threat-duration-seconds`, `respawn-delay-seconds`, `friendly-fire`, `iron-golem-neutrality`, `iron-golem-neutrality-radius`, `iron-golem-retaliation`, `notifications` et les attributs sous `attributes.*`, dont `follow-range`. Les valeurs invalides ou hors limites reviennent aux valeurs par défaut sûres.
...
- Ils ont 100 PV, 16 dégâts d’attaque, une vitesse de 0,35, une portée de poursuite de 48 blocs et une résistance au recul de 0,6 par défaut. Ces valeurs sont configurables dans `garde.attributes`. La portée effective est automatiquement relevée au moins à `garde.protection-radius` afin qu’une cible acceptée reste réellement poursuivable par l’IA.
...
- Lorsqu’un joueur protégé ou l’un de ses gardes est blessé par des dégâts non annulés, les deux gardes ciblent le véritable agresseur, y compris pour les dégâts indirects attribués au tireur. Les coups accidentels du propriétaire sont bloqués par défaut. Tous les golems de fer — naturels, gardes de village, créés par MineGus ou par un joueur — sont neutralisés par défaut (`iron-golem-neutrality: true`) : acquisition de cible annulée, cible résiduelle purgée au tick suivant, contrôle local borné une fois par seconde et dégâts annulés. Si un golem parvient malgré tout à tenter une frappe, `iron-golem-retaliation: true` ordonne au duo de contre-attaquer tout en maintenant les dégâts annulés ; cette contre-attaque de dernier recours peut être désactivée sans désactiver la neutralité. Le duo peut toujours défendre son propriétaire contre un golem. Pour autoriser entièrement les combats vanilla garde contre golem, mettre `iron-golem-neutrality: false`. La cible de combat est maintenue par une tâche globale légère quatre fois par seconde pour résister aux réévaluations de l’IA, avant d’être abandonnée après la durée configurée, ou si elle quitte le monde ou `protection-radius`.
...
- Gardes royaux : exécuter `/garde`, vérifier l’apparition de deux Husks `Garde Royale` équipés, s’éloigner de plus de 20 blocs puis changer de monde pour confirmer leur rappel sûr, tester une attaque PvP et un projectile, vérifier qu’ils n’attaquent ni leur propriétaire ni leur binôme, approcher un golem de fer naturel d’un village et confirmer qu’il perd immédiatement le garde comme cible, provoquer une tentative de frappe pour vérifier l’annulation des dégâts et la contre-attaque du duo, bloquer leur chemin pour contrôler le rappel, tuer un garde et vérifier son retour après 20 secondes, puis exécuter `/garde` à nouveau ou se déconnecter pour vérifier le renvoi et l’annulation du respawn.
